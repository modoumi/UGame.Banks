...
namespace Lobby.Flow.Services.CashServices
{
    ...
    /// 新的长版本提现条件检查服务
    /// </summary>
    public class CheckCashConditionTwo : ICheckCashCondition
    {
        public async Task CheckCashCondition(BraWithDrawIpo ipo)
        {
            //提现审批校验
            ...
            if ((ipo.Amount + ipo.HistoryUserCashInfo.SumCashAmount.Value.AToM(ipo.CurrencyId)) > ipo.UserExEo.TotalPayAmount.AToM(ipo.CurrencyId)*ipo.CashConfigEo.CashComparePayMultiple)
                throw new CustomException(FlowResponseCodes.RS_CASH_NOTMEETMINPAY, "Condition not met.2");
        }

        public async Task<bool> IsAudit(BraWithDrawIpo ipo)
        {
            //是否审批
            if (ipo.CashConfigEo.IsAudit)
                return true;
            
            if ((ipo.HistoryUserCashInfo.SumCashAmount.Value.AToM(ipo.CurrencyId)+ipo.Amount) > ipo.UserExEo.TotalPayAmount.AToM(ipo.CurrencyId) * (decimal)ipo.CashConfigEo.HistoryCashComparePayMultiple)
                return true;
            ...
            if(!userHasCash)
            {
                if (ipo.Amount >= ipo.CashConfigEo.GlobalFirstCashAuditLimit.AToM(ipo.CurrencyId))
                    return true;
            }
