...
{
    public class CallbackService : PayCallbackServiceBase<CallbackIpoCommonBase, LetsCallbackDto>
    {
        ...
        public async Task<LetsCallbackDto> PayCallback(PayInAsyncResponse ipo)
        {
            ...
            try
            {
               ...
               ipo.Order.OwnFee = ipo.fee = money /MULTIPLE * payinFee; //30
                ...
                try
                {
                    var currencyId = ipo.Order.CurrencyID;
                    ...
                    if (ipo.status == "2")
                    {
                        ...
                        //2.更新订单
                        await UpdateBankOrder(ipo, bankOrderId, ipo.fee, order.UserFee, order.UserMoney, BankOrderStatusEnum.Success, OrderTypeEnum.Charge, bankTime, tm);
                        //3.添加银行通讯日志
                    ...
                    {
                        //添加失败订单
                        await UpdateBankOrder(ipo, bankOrderId, ipo.fee, order.UserFee, order.UserMoney, BankOrderStatusEnum.Fail, OrderTypeEnum.Charge,bankTime, tm);
                        //更新银行通讯日志
                        ...
                        await MQUtil.PublishAsync(new BankErrorMsg
                        {
                            ...
                            ErrorMsg = $"orderid:{orderId},userid:{userId}充值回调失败！status:{ipo.status}",
        ...
        public async Task<LetsCallbackDto> CashCallback(PayOutAsyncResponse ipo)
        {
            ...
            try
            {
                ...
                var amount = decimal.Parse(ipo.amount);
                ipo.fee = amount / MULTIPLE * payoutFee;//10
                ...
                if (order.TransMoney.MToA(order.CurrencyID) != amount.MToA(order.CurrencyID))
                    throw new Exception($"代付金额amount:{ipo.amount}与订单金额transmoney:{order.TransMoney}不一致！orderid:{order.OrderID}");
                string bankOrderId = order.BankOrderId;
                ...
                try
                {
                    ...
                    if (!decimal.TryParse(ipo.amount, out var tejeeMoney))
